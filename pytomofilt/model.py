...
def _default_depths(dmin=_dmoho, dmax=_dcmb):
    ...
    knots_d = (dmin - dmax) / 2.0 * KNOT_depth + (dmin + dmax) / 2.0
    return knots_d
...
class RTS_Model:
    ...
    def from_file(cls, filename, dmin=_dmoho, dmax=_dcmb, knots=None):
        ...
        f.close()
        assert di == len(knots), f"End of file without seeing all expected depth, di={di}, knots={knots}"
    ...
    def reparam(self, layer_model):
        ...
        layer_depth = [l.depth for l in layer_model.layers]
        sh_coefs = np.zeros((len(layer_depth),) + self.coefs.shape[1:])
        ...
        self.coefs = spline.cubic_spline(sh_coefs, layer_depth,
                                         self.knot_splines)
    ...
    def as_vector(self):
        ...
        for di in range(len(self.knots_d)):
            for li in range(self.lmax + 1):
                for mi in range(li+1):
                    vector[counter] = self.coefs[di,0,li,mi]
                    counter = counter + 1
                    if mi != 0:
                        vector[counter] = self.coefs[di,1,li,mi]
                        counter = counter + 1
    ...
    def from_vector(self, vector):
        ...
        for di in range(len(self.knots_d)):
            for li in range(self.lmax + 1):
                for mi in range(li+1):
                    self.coefs[di,0,li,mi] = vector[counter]
                    counter = counter + 1
                    if mi != 0:
                        self.coefs[di,1,li,mi] = vector[counter]
                        counter = counter + 1
    ...
    def plot_depth_slice(self, spline_point_np):
        ...
        plotting.plot_shcoefs(self.coefs[spline_point_np],
                              d=self.knots_d[spline_point_np],
                              title='Depth slice')
    ...
    def _write_sph_body_lines(self, fhandle):
        ...
        for di in range(len(self.knots_d)):
            for li in range(self.lmax+1):
                ...
                items = 0
                for mi in range(li+1):
                    line = line + " {: 10.4E}".format(self.coefs[di,0,li,mi])
                    items = items + 1
                    if items == 11: 
                        line = line + "\n"
                        items = 0
